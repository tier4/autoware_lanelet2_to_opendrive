...
This section describes implementation limitations and behavioral differences when converting from Lanelet2 to OpenDRIVE format.

### Stop Line Position Discrepancies

**Issue**: Stop line positions may not be accurately preserved in the conversion.

**Cause**: CARLA (the primary target simulator for this tool) uses TriggerVolume-based collision detection for traffic signals, which automatically determines stop line positions. This automatic positioning can differ from the explicit stop line positions defined in Lanelet2 maps.

**Impact**: Stop lines in the resulting OpenDRIVE map may be shifted from their original Lanelet2 positions.

### Lane Width Inconsistencies

**Issue**: Lane widths in the converted OpenDRIVE map may not exactly match the original Lanelet2 map.

**Cause**: OpenDRIVE and Lanelet2 use fundamentally different geometric representations:
- **OpenDRIVE**: Reference line-based format where lanes are defined relative to a center reference line
- **Lanelet2**: Lane boundary-based format where lanes are defined by explicit left and right boundaries

The conversion process requires mathematical interpolation (including spline fitting and other geometric algorithms) to transform between these representations, making exact width preservation impossible.

**Impact**: Lane widths may vary slightly from the original Lanelet2 specification due to mathematical interpolation.

### Special Traffic Signals Not Supported

**Issue**: Special traffic signal types (e.g., arrow signals, pedestrian signals, directional signals) are not supported.

**Cause**: This tool was originally developed to support CARLA simulator integration. CARLA does not support special traffic signal types such as:
- Arrow signals (left/right/straight arrows)
- Pedestrian crossing signals
- Other directional or conditional signals

**Workaround**: If you require support for special traffic signals for other use cases, please:
1. Open an issue on the GitHub repository
2. Describe your specific use case and required signal types
3. Contact the maintainers to discuss potential implementation

**Future Support**: Support for additional traffic signal types may be added based on community demand and maintainer availability.
