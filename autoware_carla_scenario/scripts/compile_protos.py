...
class ProtoGroup:
    ...
    Attributes:
        ...
        import_pattern: Matches the absolute ``from <pkg>.v0 import`` lines protoc
            emits, so they can be rewritten to relative ``from . import``.
        init_docstring: Docstring for the generated package ``__init__.py``.
    """
...
BRIDGE_GROUP = ProtoGroup(
    ...
    # ``from autoware_bridge.v0 import x_pb2 as y`` -> ``from . import x_pb2 as y``
    import_pattern=re.compile(r"^from autoware_bridge\.v0 import ", flags=re.MULTILINE),
    init_docstring=(
        '"""Generated protobuf modules for the ``AutowareBridge`` contract.\n'
        "\n"
        "This package is generated -- see ``autoware_carla_scenario/proto/README.md``.\n"
        '"""\n'
    ),
...
def _generate(group: ProtoGroup, destination: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for path in generated:
            if path.suffix not in (".py", ".pyi"):
                continue
            text = group.import_pattern.sub("from . import ", path.read_text())
    ...
    (destination / "__init__.py").write_text(group.init_docstring)
