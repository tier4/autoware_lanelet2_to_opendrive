...
class LanePolynomial:
    """Tagged result of width-or-border decision for one lane.

    kind="width":  segments are (s_offset, a, b, c, d) of LOCAL width(s).
                   Emitted as <lane><width>.
    kind="border": segments are (s_offset, a, b, c, d) of SIGNED outer-bound
                   t(s) relative to the road reference line. Emitted as
                   <lane><border>. Mutually exclusive with width per
                   OpenDRIVE 1.4 spec.
    """
    ...
    def __post_init__(self) -> None:
        if self.kind not in ("width", "border"):
            raise ValueError(
                f"LanePolynomial.kind must be 'width' or 'border', "
                f"got {self.kind!r}"
            )
