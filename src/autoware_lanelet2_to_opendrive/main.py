...
class _Lanelet2ToOpenDRIVEConverter:
    """Helper class to organize conversion logic into smaller methods."""
    ...
    def _build_regular_roads(self) -> Tuple[List[Road], Dict[int, int]]:
        """
        ...
        regular_roads = Road.construct_from_lanelet_map(self.lanelet_map)
    ...
    def _build_junction_structure(
        self, regular_roads: List[Road], starting_junction_road_id: int
    ) -> Tuple[List[Junction], List[Road], Dict[int, int], Dict[int, List[int]]]:
        """
        Build junction structure from junction lanelets.

        Args:
            regular_roads: Already-built regular roads
            starting_junction_road_id: Starting ID for connecting roads

        Returns:
            Tuple of:
                - List of Junction objects
                - List of connecting roads (roads inside junctions)
                - Dictionary mapping junction lanelet ID to road ID
                - Dictionary mapping junction ID to list of connecting road IDs
        """
        from autoware_lanelet2_to_opendrive.junction import (
        ...
        for junction_index, junction_group in enumerate(junction_groups):
            ...
            connections = Junction.build_connections_from_roads(
                ...
                junction_id=junction_id,
                lanelet_to_road_id={},  # Will be filled in next step
        ...
        return junctions, connecting_roads, junction_lanelet_to_road, junction_to_roads

    def _build_road_lanelet_mappings(
        self,
        lanelet_to_road_id: Dict[int, int],
        junction_lanelet_to_road: Dict[int, int],
    ) -> RoadLaneletMapping:
        """
        Create bidirectional mappings between roads and lanelets.

        Args:
            lanelet_to_road_id: Mapping of regular lanelets to roads
            junction_lanelet_to_road: Mapping of junction lanelets to roads

        Returns:
            RoadLaneletMapping with bidirectional maps
        """
    ...
    def _setup_connections(
        ...
        Args:
            ...
            connecting_roads: Connecting roads (inside junctions)
            junctions: All junctions
            mapping: Road-lanelet bidirectional mappings
        """
        print("\n=== Building road links for connecting roads ===")
        Road.set_connecting_road_links(
            lanelet_map=self.lanelet_map,
            connecting_roads=connecting_roads,
            lanelet_to_road_id=mapping.lanelet_to_road,
            road_to_lanelet_ids=mapping.road_to_lanelets,
        )
    ...
    def _extract_and_assign_signals(
        self, all_roads: List[Road], junctions: List[Junction], mapping: RoadLaneletMapping
    ) -> SignalsAndControllers:
        """
        ...
        Returns:
            SignalsAndControllers object with all signals
        """
        from autoware_lanelet2_to_opendrive.junction import (
            filter_lanelets_inside_junction,
        )
    ...
    def _assign_controllers_to_junctions(
        ...
        """
        Assign controllers to junctions.
    ...
    def _write_opendrive_output(
        ...
        """
        Create OpenDRIVE object and optionally save to file.

        Args:
            header: OpenDRIVE header
    ...
    def convert(self) -> Tuple[OpenDRIVE, RoadLaneletMapping]:
        ...
        Returns:
            Tuple of OpenDRIVE object and RoadLaneletMapping
        """
        ...
        (
            ...
            junction_lanelet_to_road,
            junction_to_roads,
        ) = self._build_junction_structure(regular_roads, starting_junction_road_id)

        # Step 3: Create bidirectional mappings
        mapping = self._build_road_lanelet_mappings(
            lanelet_to_road_id, junction_lanelet_to_road
        )

        # Update junctions with complete lanelet-to-road mapping
        from autoware_lanelet2_to_opendrive.junction import (
            filter_lanelets_inside_junction,
            find_junction_groups,
        )
        ...
        junction_lanelets = filter_lanelets_inside_junction(
            list(self.lanelet_map.laneletLayer),
            exclude_lanelet_ids=self.config.no_junction_lanelet_ids,
        )
        junction_groups = find_junction_groups(junction_lanelets)

        for junction_index, junction_group in enumerate(junction_groups):
            junction_id = junction_index + self.config.junction_id_offset
            connecting_road_ids = junction_to_roads.get(junction_id, [])
            all_roads_for_junction = regular_roads + connecting_roads
            connections = Junction.build_connections_from_roads(
                lanelet_map=self.lanelet_map,
            ...
            junctions[junction_index].connections = connections
        ...
        self._setup_connections(all_roads, connecting_roads, junctions, mapping)

        # Step 5: Extract and assign signals
        signals_and_controllers = self._extract_and_assign_signals(
            all_roads, junctions, mapping
        )

        # Step 6: Assign controllers to junctions
        ...
        opendrive = self._write_opendrive_output(
            header, all_roads, junctions, signals_and_controllers
        )
